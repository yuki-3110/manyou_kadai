  ...
  describe '一覧表示機能' do
    context '一覧画面に遷移した場合' do
      it '作成済みのタスク一覧が表示される' do
        ...
        second_task = FactoryBot.create(:second_task, title: 'task', content: '買い物', deadline: '2022-07-06', status: '未着手', priority: '高')
        visit tasks_path
        expect(page).to have_content 'task'
      end
    end
    context 'タスクが作成日時の降順に並んでいる場合' do
      it '新しいタスクが一番上に表示される' do
        task = FactoryBot.create(:task, title: 'task1', content: '買い物', deadline: '2022-07-06', status: '未着手', priority: '高')
        second_task = FactoryBot.create(:second_task, title: 'task2', content: '買い物', deadline: '2022-07-06', status: '未着手', priority: '高')
        visit tasks_path
